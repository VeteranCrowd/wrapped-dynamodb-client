    ...
    describe('create ... delete', function () {
      ...
      describe('items', function () {
        describe('validations', function () {
          ...
          it('transactPuts/transactDeletes with condition expressions should close', async function () {
            const entityPK = nanoid();
            const items = _.range(26).map((entitySK) => ({
              entityPK,
              entitySK,
            ...
            // Put items with condition expressions.
            let response = await baseClient.transactPutItems(
              tableName,
              items,
              items.map((item) => ({
                expression:
                  'attribute_not_exists(entityPK) AND attribute_not_exists(entitySK)',
