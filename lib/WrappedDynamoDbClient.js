...
import { buildConditionExpression } from './conditionExpression.js';
...
export class WrappedDynamoDbClient {
      ...
      await Promise.allSettled(
        chunks.map(async (chunk) => {
          ...
          while (chunk.length) {
            ...
            chunk = response?.UnprocessedItems?.[tableName]?.map((r) => r?.DeleteRequest?.Key) ?? [];
            if (chunk.length) {
              this.#logger.debug(`${chunk.length} items not processed...`,
                {
                  keys: chunk,
                }
              );
   ...
   * @param {object} [conditionExpressions] - Condition expression object.
   ...
   * @category item
   */
  async putItem(tableName, item, conditionExpressions) {
    // Validate arguments.
    this.#validateTableName(tableName);
    ...
    if (conditionExpressions) {
      const { ConditionExpression, ExpressionAttributeNames, ExpressionAttributeValues } = buildConditionExpression(conditionExpressions);
    ...
    try {
      const response = await this.#doc.put(params);
      ...
      if (!(err instanceof ConditionalCheckFailedException)) {
        this.#logger?.error(`Failed to put item to table ${tableName}.`, err);
      }
   ...
   * @param {number} [options.concurrency] - Max concurrent batchWrite calls in flight (default 5). Lower this for bulk writes against a capacity-constrained table.
   * @param {number} [options.maxRetries] - Max backoff retries per chunk on throttling before failing (default 5).
   * @return {Promise<Array>} Array of responses from chunked batchWrite operations.
   * @category item
   */
  async putItems(tableName, items, { concurrency = 5, maxRetries = 5 } = {}) {
    this.#validateTableName(tableName);
    ...
    this.#logger.debug("Options", {
      concurrency,
      maxRetries,
      itemsCountPerBatch: batchSize * concurrency,
    });


    // Exponential backoff with full jitter so concurrent chunks fan out
    ...
    // Items that could not be written after exhausting maxRetries. Collected
    // across all workers (single-threaded, so plain push is safe) and logged at
    // the end so a caller can re-drive them instead of losing them silently.
    const failedItems = [];

    // Write a single <=25-item chunk, retrying on both throttle signals until
    // it fully succeeds or exhausts maxRetries. Non-throttle errors propagate to
    // the caller with context; throttle exhaustion records the chunk's items.
    const putChunk = async (initialChunk) => {
      ...
      while (chunk.length) {
        ...
        try {
          this.#logger.debug(`Putting items to table ${tableName} started ...`);

          response = await this.#doc.batchWrite({
            RequestItems: { [tableName]: requests },
          });
        } catch (error) {
          if (!this.isThrottleError(error)) {
            throw new Error(
              `Failed to put items to table ${tableName}: ${error.message}`,
            );
          }

          if (attempt >= maxRetries) {
            // Throttled past the retry limit — record the keys and give up on
            // this chunk so the rest of the migration can proceed.
        ...
        // Re-submit any items DynamoDB declined to process (partial throttle).
        chunk =
        ...
        if (chunk.length) {
          ...
          this.#logger.debug(
            `${chunk.length} items not processed, retrying...`,
            {
    ...
    // Bounded-concurrency runner: keep at most `concurrency` batchWrite calls
    // in flight rather than firing every chunk at once (the previous behaviour,
    // which burst past target-table write capacity).
    ...
    const worker = async () => {
      while (next < chunks.length) {
        ...
        results[index] = await putChunk(chunks[index]);
      }
    };
    ...
    this.#logger.debug(`Put items to table ${tableName} finished.`);

    if (failedItems.length) {
      const failedKeys = failedItems.map((item) => ({
        entityPK: item?.entityPK,
        entitySK: item?.entitySK,
      }));

      this.#logger.error(
        `Failed to put ${failedItems.length} items to table ${tableName} after ${maxRetries} retries.`,
        { failedKeys },
      );
    }
  ...
  async scan(tableName, options = {}) {
    ...
    if (response.$metadata.httpStatusCode === 200)
      this.#logger.debug(`Scanned table ${tableName}.`, {
  ...
  async transactPutItems(
    ...
    applyConditionExpressionsToAllItems = false,
    getExpressionCb = (item) =>
      conditionExpressions.find(
        (expr) =>
          expr?.entityPK === item.entityPK && expr?.entitySK === item.entitySK,
      ),
  ) {
    this.#validateTableName(tableName);
    this.#validateItems(items);
    ...
    if (
      ...
      !_.isPlainObject(conditionExpressions)
    ) {
      throw new Error(
        'conditionExpressions must be an object when applyConditionExpressionsToAllItems is true',
      );
    } else if (
      !applyConditionExpressionsToAllItems &&
      !_.isArray(conditionExpressions)
    ) {
      throw new Error(
        'conditionExpressions must be an array when applyConditionExpressionsToAllItems is false',
      );
    }
  ...
  isThrottleError(error) {
    if (error instanceof ProvisionedThroughputExceededException || error instanceof RequestLimitExceeded || error.name === 'ThrottlingException') {
      return true;
    }
