...
export class WrappedDynamoDbClient {
  ...
  async putItem(tableName, item) {
    ...
    const response = await this.#doc.put({
      TableName: tableName,
      Item: item,
    });
   ...
   * @param {boolean} [applyQueryToAllItems=false] - If true, apply the same condition expression to all items. Default false.
   * @param {Function} [getQueryCb] - Callback function to find the condition expression for each item.
   * @return {Promise<Array>} Array of responses from chunked batchWrite operations.
  ...
  async transactPutItems(
    ...
    if (applyQueryToAllItems && !_.isObject(conditionExpressions)) {
      throw new Error(
        'conditionExpressions must be an object when applyQueryToAllItems is true',
      );
    } else if (!applyQueryToAllItems && !_.isArray(conditionExpressions)) {
      throw new Error(
        'conditionExpressions must be an array when applyQueryToAllItems is false',
      );
    }
    ...
    const result = await this.#doc.transactWrite({
      TransactItems: items.map((item) => {
        const { expression } = applyQueryToAllItems
          ? (conditionExpressions ?? {})
          : (getQueryCb(item) ?? {});
